...
fn get_test_user_info_resource(
  test_resources: &utils::TestResources,
) -> qv::models::UserInfoResource {
  test_resources
    .http_client
    .get(&format!(
      "{}{}",
      test_resources.base_url, "/private/user-info"
    ))
...
fn patch_poll_integration() {
  ...
  let create_poll_resource: qv::models::PollResource = test_resources
    .http_client
  ...
  assert_eq!(create_poll_resource.poll.title, create_poll_payload.title);

  let patch_poll_payload = qv::models::PatchPollPayload {
    title: Option::from("patched title".to_string()),
    full_description_link: Option::None,
    summary: Option::None,
  };

  let patch_poll_resource: qv::models::PollResource = test_resources
    .http_client
    .patch(&format!(
      "{}{}{}",
      test_resources.base_url, "/private/polls/", create_poll_resource.poll.id
    ))
    .header("Authorization", utils::DEBUG_TOKEN_1)
    .json(&patch_poll_payload)
    .send()
    .unwrap()
    .json()
    .unwrap();
  ...
  assert_eq!(
    patch_poll_resource.poll.title,
    patch_poll_payload.title.unwrap()
  );

  assert_eq!(
    patch_poll_resource.poll.summary,
    create_poll_payload.summary
  );
}

#[test]
fn end_to_end_integration() {
  let test_resources = utils::build_test_resources();
  let user_info_resource = get_test_user_info_resource(&test_resources);
  ...
  assert_eq!(
    create_poll_resource.poll.email,
    user_info_resource.user.email
  );

  assert_eq!(
    create_poll_resource.poll.current_progress,
    qv::sql_enum_types::ProgressEnum::NotStarted
  );
  ...
  let create_proposal_resource: qv::models::CreateProposalResource = test_resources
    ...
    .post(&format!(
      "{}{}{}{}",
      test_resources.base_url, "/private/polls/", create_poll_resource.poll.id, "/proposals"
    ))
    .header("Authorization", utils::DEBUG_TOKEN_1)
  ...
  assert_eq!(
    create_proposal_resource.proposal.poll_id,
    create_poll_resource.poll.id
  );
  ...
  let invite_user_response: reqwest::Response = test_resources
    ...
    .post(&format!(
      "{}{}{}{}",
      test_resources.base_url, "/private/polls/", create_poll_resource.poll.id, "/invite-user"
    ))
    .header("Authorization", utils::DEBUG_TOKEN_1)
  ...
  let start_poll_response: reqwest::Response = test_resources
    ...
    .put(&format!(
      "{}{}{}{}",
      test_resources.base_url, "/private/polls/", create_poll_resource.poll.id, "/start"
    ))
    .header("Authorization", utils::DEBUG_TOKEN_1)
    .json(&invite_user_payload)
    .send()
  ...
  let create_vote_payload1 = qv::models::CreateVotePayload { points: 9.0 };

  let vote_response1: reqwest::Response = test_resources
    .http_client
    .put(&format!(
      "{}{}{}{}",
      test_resources.base_url, "/private/proposals/", create_proposal_resource.proposal.id, "/vote"
    ))
  ...
  let create_vote_payload2 = qv::models::CreateVotePayload { points: -4.0 };
  ...
  assert!(home_resource
    .polls
    .iter()
    .cloned()
    .find(|p| p.id == create_poll_resource.poll.id)
    .is_some());
  ...
  let finish_response: reqwest::Response = test_resources
    ...
    .put(&format!(
      "{}{}{}{}",
      test_resources.base_url, "/private/polls/", create_poll_resource.poll.id, "/finish"
    ))
    .header("Authorization", utils::DEBUG_TOKEN_1)
    .send()
    .unwrap();
  ...
  let finished_get_poll_resource: qv::models::GetPollResource =
    finished_get_poll_response.json().unwrap();

  assert_eq!(finished_get_poll_resource.point_totals.is_some(), true);
  assert_eq!(
    finished_get_poll_resource
      .point_totals
      .unwrap()
      .get(&create_proposal_resource.proposal.id)
      .unwrap(),
    &1.0
  );
