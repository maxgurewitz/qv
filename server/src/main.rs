...
fn map_to_internal_service_err<T: Display>(e: T) -> HttpResponse {
  ...
  HttpResponse::InternalServerError().json(GenericJsonResponse {
    message: INTERNAL_SERVICE_ERROR_MSG.to_string(),
  })
}
...
fn create_poll_route(
  ...
  req: HttpRequest,
) -> Result<Json<PollResource>, Error> {
  use schema::polls;

  let connection = data.pg_pool.get().map_err(map_to_internal_service_err)?;
  ...
  let new_poll = NewPoll {
    ...
    title: &payload.title,
    // TODO add validation. default "qv"
...
fn get_poll_route(
  ...
  use schema::{polls, proposals, votes};
  ...
  let poll = polls::table
    ...
    .map_err(map_to_internal_service_err)?
    .ok_or_else(|| {
      HttpResponse::NotFound().json(GenericJsonResponse {
  ...
  let point_totals = if poll.current_progress == sql_enum_types::ProgressEnum::Finished {
    ...
    for vote in &vote_points {
      ...
      let sum_for_proposal = totals.entry(proposal_id).or_insert(0.0);
...
fn delete_proposal_route(
  ...
  Ok(HttpResponse::Ok().json(GenericJsonResponse {
...
fn patch_poll_route(
  ...
  let poll_id = poll_id_param.into_inner();
  let target = polls.filter(id.eq(poll_id));
  let patch = payload.into_inner();

  let poll = diesel::update(target)
    .set(&patch)
    .get_result::<Poll>(&*connection)
    .map_err(|e| match e {
      diesel::result::Error::NotFound => HttpResponse::Forbidden().json(GenericJsonResponse {
        message: "Poll not found".to_string(),
      }),
...
fn assign_vote_points_route(
  ...
  use schema::{polls, proposals, user_invite_locks, user_invites, votes};
  let proposal_id = proposal_id_param.into_inner();
  ...
  connection
    .transaction::<_, diesel::result::Error, _>(|| {
      ...
      let within_budget = (vote_sum - vote_to_overwrite_option.map(|v| v.points).unwrap_or(0.0)
        + payload.points)
        < 100.0;

      if poll_progress == sql_enum_types::ProgressEnum::InProgress && within_budget {
        vote_to_overwrite_option
          .map(|vote_to_overwrite| {
...
fn start_poll(
  ...
  let target = id
    .eq(inner_poll_id)
    .and(current_progress.eq(sql_enum_types::ProgressEnum::NotStarted));
...
fn finish_poll(
  ...
  let target = id
    .eq(poll_id)
    .and(current_progress.eq(sql_enum_types::ProgressEnum::InProgress));
...
fn invite_user(
  ...
  use schema::{user_invite_locks, user_invites};
  ...
  Ok(
    HttpResponse::Ok()
      ...
      .json(GenericJsonResponse {
        message: "Succesfully invited user.".to_string(),
      }),
  )
}
          ...
          web::scope("/private")
              ...
              web::scope("/polls")
                ...
                .service(
                  web::scope("/{poll_id}")
                    ...
                    .route("", web::patch().to(patch_poll_route))
            ...
            .service(
              web::scope("/proposals/{proposal_id}")
                .route("", web::patch().to(patch_proposal_route))
